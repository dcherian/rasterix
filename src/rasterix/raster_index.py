...
def _get_1d_index_size(index: PandasIndex | AxisAffineTransformIndex) -> int:
    if isinstance(index, PandasIndex):
        return index.index.size
    elif isinstance(index, AxisAffineTransformIndex):
        return len(index)
    else:
        raise TypeError(f"Can't estimate length of index of type {type(index).__name__!r}")
...
def assign_index(obj: T_Xarray, *, x_dim: str | None = None, y_dim: str | None = None) -> T_Xarray:
    ...
    index = RasterIndex.from_transform(
        obj.rio.transform(), obj.sizes[x_dim], obj.sizes[y_dim], x_dim=x_dim, y_dim=y_dim, crs=obj.proj.crs
    )
...
class AxisAffineTransformIndex(CoordinateTransformIndex):
    ...
    def __len__(self) -> int:
        return self.axis_transform.size
...
class RasterIndex(Index, xproj.ProjIndexMixin):
    ...
    def __init__(self, indexes: Mapping[WrappedIndexCoords, WrappedIndex], crs: CRS | Any | None = None):
        ...
        if axis_independent:
            self._shape = {}
            for dim in ("x", "y"):
                if index := self._wrapped_indexes.get(dim, None):
                    self._shape[dim] = _get_1d_index_size(index)
        else:
